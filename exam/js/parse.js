...
$('#output').on('click', '.answer-item', function() {
...
function parseanswer() {
	var _an = $('#answer-input').val().split('**');
	...
	for (var i = 0, n = _s.length; i < n; i++) {
		var index = i+1;
		str1 = str1 +'<span class="answer-item">' + index + '. ' + _s[i].toUpperCase() + '</span>';
	}
	...
	for (var i = 0, n = _m.length; i < n; i++) {
		var index = i+1;
		str2 = str2 + '<span class="answer-item">' + index + '. ' + _m[i].toUpperCase() + '</span>';
	}
	...
	for (var i = 0, n = _s2.length; i < n; i++) {
		var index = i+1;
		str3 = str3 +'<span class="answer-item">' + index + '. ' + _s2[i].toUpperCase() + '</span>';
	}
...
function updateWrongNum() {
	$('#wrong-num').text($('.answer-item.red').length);
}